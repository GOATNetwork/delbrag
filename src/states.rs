...
type InnerResult<S> = Result<(S, Msg), Error>;
...
impl EvalStep1 {
    fn run(mut self, msg: &[u8], circuit: &Circuit) -> InnerResult<EvalStep2> {
...
impl ContribStep1 {
    fn run(self, msg: &[u8]) -> InnerResult<ContribStep1a> {
...
impl EvalStep2 {
    fn run(self, msg: &[u8]) -> InnerResult<EvalStep2a> {
        let (msg1, msg2): (Msg, Msg) = deserialize(msg)?;
...
impl ContribStep1a {
    fn run(self, msg: &[u8], circuit: &Circuit) -> InnerResult<ContribStep2> {
        let (msg1, msg2): (Msg, Msg) = deserialize(msg)?;
...
impl EvalStep2a {
    fn run(self, msg: &[u8], circuit: &Circuit) -> InnerResult<EvalStep3> {
        let (msg1, msg2): (Msg, Msg) = deserialize(msg)?;
...
impl ContribStep2 {
    ...
    fn run(self, msg: &[u8]) -> InnerResult<ContribStep3> {
...
impl EvalStep3 {
    fn run(self, msg: &[u8]) -> InnerResult<EvalStep4> {
...
impl ContribStep3 {
    fn run(self, msg: &[u8]) -> InnerResult<ContribStep4> {
        let (msg1, msg2): (Msg, Msg) = deserialize(msg)?;
...
impl EvalStep4 {
    fn run(self, msg: &[u8]) -> InnerResult<EvalStep5> {
        let (msg1, msg2): (Msg, Msg) = deserialize(msg)?;
...
impl ContribStep4 {
    fn run(self, msg: &[u8], circuit: &Circuit) -> InnerResult<AndsBucketingState> {
        let (msg1, msg2): (Msg, Msg) = deserialize(msg)?;
...
impl EvalStep5 {
    fn run(self, msg: &[u8], circuit: &Circuit) -> InnerResult<EvalStep6> {
        let (msg1, msg2): (Msg, Msg) = deserialize(msg)?;
...
impl ContribBucketingStep {
    fn run(self, msg: &[u8], circuit: &Circuit, input: &[bool]) -> InnerResult<InputProcContrib> {
        let (msg1, msg2): (Msg, Msg) = deserialize(msg)?;
...
impl EvalStep6 {
    fn run(self, msg: &[u8], circuit: &Circuit, input: &[bool]) -> InnerResult<InputProcEval> {
        let (msg1, msg2): (Msg, Msg) = deserialize(msg)?;
...
fn init_ot2(mut state: OtInitState1, msg: &[u8]) -> StateResult<OtInitState2> {
    ...
    let coin_msg = cointossing::serialize(&state.coin_share)?;
...
fn init_ot3(state: OtInitState2, msg: &[u8]) -> StateResult<OtInitState3> {
    ...
    let coin =
        cointossing::finish(state.coin_share, state.coin_commitment, upstream_coin)?;
    let ot_init = serialized_ot_init.deserialize()?;
...
fn ot_ands4(mut state: OtAndsState3, msg: &[u8]) -> StateResult<OtAndsState4> {
    ...
    for i in (0..and_bits.len()).step_by(3) {
        ...
        let rand_for_eq_box_hash = KeyType(state.rng.r#gen());
    ...
    for (idx, gate) in circuit.gates().iter().enumerate() {
        match gate {
            Gate::InContrib | Gate::InEval | Gate::And { .. } => {
                ...
                abit_offset += 1;
                masks[idx].label_0 = (rng.r#gen::<u128>()).into();
            }
            _ => {}
        }
    }
...
impl InputProcContrib {
    fn run(mut self, msg: &[u8], circuit: &Circuit, input: &[bool]) -> InnerResult<()> {
...
impl InputProcEval {
    fn run(mut self, msg: &[u8], circuit: &Circuit) -> InnerResult<Vec<bool>> {
