...
impl LeakyOtReceiver {
    ...
    pub(crate) fn new_batch(
        ...
        for (i, t_i) in t_i.iter_mut().enumerate() {
            *t_i = KeyType(self.otg0[i].r#gen::<u128>());
        }

        for i in 0..BLOCK_SIZE {
            ot_out[i] = MacType(t_i[i].0 ^ self.otg1[i].r#gen::<u128>() ^ random_bits);
        }
...
fn test_serialization() {
    ...
    let delta = Delta(rng_send.r#gen());
